...
from allauth.utils import get_user_model, get_request_param
...
class SocialApp(models.Model):
    ...
    provider = models.CharField(
        verbose_name=_("Nhà cung cấp dịch vụ"),
        max_length=30,
        choices=registry.as_choices(),
    )
    name = models.CharField(verbose_name=_("Tên ứng dụng"), max_length=40)
    client_id = models.CharField(
        verbose_name=_("ID ứng dụng dùng đăng nhập"),
        max_length=191,
        help_text=_("ID ứng dụng hoặc khóa khách hàng"),
    )
    secret = models.CharField(
        verbose_name=_("Khóa bí mật của ứng dụng"),
        max_length=191,
        blank=True,
        help_text=_("API secret, client secret, or" " consumer secret"),
    )
    key = models.CharField(
        verbose_name=_("Khóa"), max_length=191, blank=True, help_text=_("Khóa")
    )
    ...
    class Meta:
        verbose_name = _("Ứng dụng mạng xã hội")
        verbose_name_plural = _("Ứng dụng mạng xã hội")
...
class SocialAccount(models.Model):
    ...
    uid = models.CharField(
        verbose_name=_("uid"), max_length=getattr(settings, "SOCIALACCOUNT_UID_MAX_LENGTH", 191)
    )
    last_login = models.DateTimeField(verbose_name=_("Đăng nhập lần cuối"), auto_now=True)
    date_joined = models.DateTimeField(verbose_name=_("Ngày tham gia"), auto_now_add=True)
    extra_data = JSONField(verbose_name=_("Ghi chú thêm"), default=dict)

    class Meta:
        unique_together = ("provider", "uid")
        verbose_name = _("Tài khoản mạng xã hội")
        verbose_name_plural = _("Tài khoản mạng xã hội")
...
class SocialToken(models.Model):
    ...
    token = models.TextField(
        ...
        help_text=_('"oauth_token" (OAuth1) hoặc token truy cập (OAuth2)'),
    )
    token_secret = models.TextField(
        blank=True,
        verbose_name=_("token secret"),
        help_text=_('"oauth_token_secret" (OAuth1) hoặc token mới (OAuth2)'),
    )
    expires_at = models.DateTimeField(
        blank=True, null=True, verbose_name=_("Ngày hết hạn")
    )

    class Meta:
        unique_together = ("app", "account")
        verbose_name = _("Mã token ứng dụng xã hội")
        verbose_name_plural = _("Mã token ứng dụng xã hội")
